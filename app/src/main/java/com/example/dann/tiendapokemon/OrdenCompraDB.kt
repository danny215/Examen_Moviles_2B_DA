...
import com.example.dann.tiendapokemon.R.string.apellido
import com.example.dann.tiendapokemon.R.string.nombre
...
class OrdenCompraDB{
    companion object {
        fun insertarOrden(ordenCompraKotlinClass: OrdenCompraKotlinClass){
            "http://192.168.100.28:1337/OrdenCompra".httpPost(listOf("cedulaIdentidad" to ordenCompraKotlinClass.cedulaComprador, "nombre" to ordenCompraKotlinClass.nombre,
        ...
        fun insertarOrdenDetalles(ordenDetalles: OrdenDetalles){
            "http://192.168.100.28:1337/OrdenDetalle".httpPost(listOf("fechaEnvio" to ordenDetalles.fechaEnvio, "precio" to ordenDetalles.precio, "idPokemon" to ordenDetalles.idPokemon))
                    .responseString { request, _, result ->
                        Log.d("http-ejemplo", request.toString())
                    }
        }
        ...
        fun getOrdenesList(): ArrayList<OrdenCompraKotlinClass> {
            ...
            val (request, response, result) = "http://192.168.100.28:1337/OrdenCompra".httpGet().responseString()
